...
Builds the Expo Android APK inside Docker.
...
if [ "$MODE" = "release" ]; then
  ...
  PREPARE="cp /workspace/app/build/release/keystore.properties android/keystore.properties
node /workspace/app/scripts/patch-release-signing.js"
  GRADLE_TASKS="assembleRelease"
fi

CLEAN_CMD=""
if [ "$CLEAN" = true ]; then
  CLEAN_CMD="rm -rf app/android"
fi
...
if [ "$MODE" = "release" ]; then
  ...
  UNSIGNED_APK="app/android/app/build/outputs/apk/release/app-release-unsigned.apk"
  if [ -f "$APK" ]; then
    cp "$APK" "$OUT_DIR/"
